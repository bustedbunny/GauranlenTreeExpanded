...
namespace GauranlenTreeExpanded
{
    ...
    {
        private CompTreeConnectionExpanded connection;
        ...
        public override GizmoResult GizmoOnGUI(Vector2 topLeft, float maxWidth, GizmoRenderParms parms)
        {
            ...
            float num = 0;
            foreach (Pawn pawn in connection.ConnectedPawns)
            {
                num += connection.PruningHoursToMaintain(DesiredConnectionStrength, pawn);
            }
            Widgets.Label(rect3.x, ref curY, rect3.width, "PruningHoursToMaintain".Translate() + ": " + (num/connection.ConnectedPawns.Count).ToString("F1"));
            Text.Font = GameFont.Small;
        ...
        {
            string text = "";
            if (connection.ConnectedPawns.Count != 1)
            {
                text = "DesiredConnectionStrengthDescNew".Translate(connection.parent.Named("TREE"), connection.ConnectionStrengthLossPerDayAll().ToStringPercent().Named("FALL")).Resolve();
            }
            else
            {
                text = "DesiredConnectionStrengthDesc".Translate(connection.parent.Named("TREE"), connection.ConnectedPawns[0].Named("CONNECTEDPAWN"), connection.ConnectionStrengthLossPerDay(connection.ConnectedPawns[0]).ToStringPercent().Named("FALL")).Resolve();
